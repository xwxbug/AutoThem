...
//	bool			m_bWinSearchFoundFlag;							// Temp var used in Window searches
	...
	void			SetFuncErrorCode(int nCode);;					// Set script error info (@error code)
	void			SetFuncExtCode(int nCode);;						// Set script extended info (@extended code)
...
////
	...
	void			Parser_StartWithVariable(VectorToken &vLineToks, unsigned long &ivPos);
	...
	void			Parser_Keyword_ENDIF(VectorToken &vLineToks, unsigned long &ivPos);
	void			Parser_Keyword_WHILE(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_WEND(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_EXITLOOP(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_CONTINUELOOP(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_DO(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_UNTIL(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_FOR(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_NEXT(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_SELECT(VectorToken &vLineToks, unsigned long &ivPos, int &nScriptLine);
	void			Parser_Keyword_CASE(int &nScriptLine);
	void			Parser_Keyword_ENDSELECT(VectorToken &vLineToks, unsigned long &ivPos);
	void			Parser_Keyword_DIM(VectorToken &vLineToks, unsigned long &ivPos, int nReqScope);
	void			Parser_Keyword_CONST(VectorToken &vLineToks, unsigned long &ivPos, int nReqScope);
	void			Parser_Keyword_RETURN(VectorToken &vLineToks, unsigned long &ivPos);
	void			Parser_Keyword_EXIT(VectorToken &vLineToks, unsigned long &ivPos);
	...
	AUT_RESULT		Parser_EvaluateVariable(VectorToken &vLineToks, unsigned long &ivPos, Variant &vResult);
	AUT_RESULT		Parser_EvaluateMacro(const char *szName, Variant &vResult);
	AUT_RESULT		Parser_EvaluateCondition(VectorToken &vLineToks, unsigned long &ivPos, bool &bResult);
	AUT_RESULT		Parser_EvaluateExpression(VectorToken &vLineToks, unsigned long &ivPos, Variant &vResult);
	AUT_RESULT		Parser_OprReduce(StackInt &opStack, StackVariant &valStack);
	AUT_RESULT		Parser_SkipBoolean(VectorToken &vLineToks, unsigned long &ivPos);
